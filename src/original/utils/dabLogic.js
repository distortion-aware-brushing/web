...
const RELOCATION_STEP_RATIO = 0.3;
const INWARD_RELOCATION_STEP_RATIO = 0.15;
const EPSILON = 1e-6;
...
function copyPoint(point) {
	return isFinitePoint(point) ? [point[0], point[1]] : [0, 0];
}

function safeCloseness(value) {
	return Number.isFinite(value) ? value : 0;
}

function averagePoint(points, fallback = [0, 0]) {
	const finitePoints = points.filter(isFinitePoint);
	if (finitePoints.length === 0) return copyPoint(fallback);
	const sum = finitePoints.reduce((acc, point) => [acc[0] + point[0], acc[1] + point[1]], [0, 0]);
	return [sum[0] / finitePoints.length, sum[1] / finitePoints.length];
}

function getRelocationCenter(hull, brushedPositions) {
	const finiteHull = Array.isArray(hull) ? hull.filter(isFinitePoint) : [];
	if (finiteHull.length >= 3 && Math.abs(d3.polygonArea(finiteHull)) > EPSILON) {
		const centroid = d3.polygonCentroid(finiteHull);
		if (isFinitePoint(centroid)) return centroid;
	}
	return averagePoint(brushedPositions, averagePoint(finiteHull));
}

function getMaxRadius(points, center, fallbackRadius) {
	const finiteDistances = points
		.filter(isFinitePoint)
		.map(point => getDist(point[0], point[1], center[0], center[1]))
		.filter(Number.isFinite);
	if (finiteDistances.length === 0) return fallbackRadius;
	return Math.max(fallbackRadius, ...finiteDistances);
}

function getDirectionFromCenter(point, center, ordinal, total) {
	const dx = point[0] - center[0];
	const dy = point[1] - center[1];
	const dist = Math.sqrt(dx * dx + dy * dy);
	if (Number.isFinite(dist) && dist > EPSILON) return [dx / dist, dy / dist];
	const angle = (2 * Math.PI * ordinal) / Math.max(total, 1);
	return [Math.cos(angle), Math.sin(angle)];
}

function capMove(from, target, maxStep) {
	if (!isFinitePoint(target)) return copyPoint(from);
	const dx = target[0] - from[0];
	const dy = target[1] - from[1];
	const dist = Math.sqrt(dx * dx + dy * dy);
	if (!Number.isFinite(dist) || dist <= maxStep || dist <= EPSILON) return copyPoint(target);
	const ratio = maxStep / dist;
	return [from[0] + dx * ratio, from[1] + dy * ratio];
}

function capRadialMove(from, target, center, maxOutwardStep, maxInwardStep) {
	if (!isFinitePoint(target)) return copyPoint(from);
	const fromRadius = getDist(from[0], from[1], center[0], center[1]);
	const targetRadius = getDist(target[0], target[1], center[0], center[1]);
	const maxStep = targetRadius < fromRadius ? maxInwardStep : maxOutwardStep;
	return capMove(from, target, maxStep);
}

function normalizeCloseness(value, minCloseness, maxCloseness) {
	if (!Number.isFinite(value)) return 0;
	if (maxCloseness - minCloseness <= EPSILON) return 0.5;
	return (value - minCloseness) / (maxCloseness - minCloseness);
}

function isUsableHull(hull) {
	return Array.isArray(hull) && hull.filter(isFinitePoint).length >= 3 && Math.abs(d3.polygonArea(hull.filter(isFinitePoint))) > EPSILON;
}
...
export function findRelocationPositionsHull(
	...
	*/
	...
	const brushedIndices = Array.from(seedPoints ?? []).filter(i => Number.isInteger(i) && isFinitePoint(currentLd[i]));
	const brushedIndexSet = new Set(brushedIndices);
	const brushedPositions = brushedIndices.map(i => currentLd[i]);
	const maxRelocationStep = painterRadius * RELOCATION_STEP_RATIO;
	const maxInwardStep = painterRadius * INWARD_RELOCATION_STEP_RATIO;
	const finiteHull = Array.isArray(hull) ? hull.filter(isFinitePoint) : [];
	const usableHull = isUsableHull(finiteHull);
	const center = getRelocationCenter(finiteHull, brushedPositions);
	const hullRadius = getMaxRadius(finiteHull, center, getMaxRadius(brushedPositions, center, painterRadius));
	const innerRadius = Math.max(1, Math.min(hullRadius * 0.35, painterRadius * 0.5));
	const outerRadius = Math.max(hullRadius + painterRadius * 0.5, innerRadius + painterRadius * 0.5);
	const brushedCloseness = brushedIndices.map(i => safeCloseness(closenessArr?.[i]));
	const minCloseness = brushedCloseness.length > 0 ? d3.min(brushedCloseness) : 0;
	const maxCloseness = brushedCloseness.length > 0 ? d3.max(brushedCloseness) : 1;
	const brushedOrder = new Map(brushedIndices.map((idx, order) => [idx, order]));
	const bisectors = usableHull ? getBisectors(finiteHull) : [];
	const extendedHull = usableHull ? extendHull(finiteHull, painterRadius * 2) : [];
	
	const relocatedLd = currentLd.map((pos, i) => {
		const currentPos = copyPoint(pos);
		
		if (brushedIndexSet.has(i)) {
			const closeness = safeCloseness(closenessArr?.[i]);
			const normalizedCloseness = normalizeCloseness(closeness, minCloseness, maxCloseness);
			const direction = getDirectionFromCenter(
				currentPos, center, brushedOrder.get(i) ?? 0, brushedIndices.length
			);
			const desiredRadius = innerRadius + (1 - normalizedCloseness) * (outerRadius - innerRadius);
			const target = [
				center[0] + direction[0] * desiredRadius,
				center[1] + direction[1] * desiredRadius
			];
			return capRadialMove(currentPos, target, center, maxRelocationStep, maxInwardStep);
		}
		else {
			if (!usableHull) return currentPos;
			const slopeInfo = findSlopeBasedOnHull(
				currentPos[0], currentPos[1], bisectors, finiteHull, painterRadius
			);
			let slope = slopeInfo.slope;
			const hullStartPoint = slopeInfo.hullStartPoint;
			const closeness = safeCloseness(closenessArr?.[i]);
			let target;
			if (closeness >= 1 - EPSILON) {
				if (isPointWithinHull(currentPos[0], currentPos[1], finiteHull)) {
					return currentPos;
				}
				target = [hullStartPoint[0], hullStartPoint[1]];
			}
			else if (closeness <= EPSILON) {
				if (!isPointWithinHull(currentPos[0], currentPos[1], extendedHull)) {
					return currentPos;
				}
				target = [hullStartPoint[0] + slope[0] * 3, hullStartPoint[1] + slope[1] * 3];
			}
			else {
				target = [
					hullStartPoint[0] + slope[0] * (2 - 2 * closeness),
					hullStartPoint[1] + slope[1] * (2 - 2 * closeness)
				];
			}
			return capMove(currentPos, target, maxRelocationStep);
		}
	});
	return relocatedLd;
}
