...
class MultiDBrushing {
	...
	constructor(
		...
		this.timer = true;
		this.relocationStartLd = undefined;
	...
	parser() {
		 ...
		 */
		const csr = this.preprocessed.csr ?? this.preprocessed.similarity;
	...
	initiateRelocationDuringBrushing(perform = true) {
		this.relocationStartLd = this.currLd.map(pos => [pos[0], pos[1]]);
		if (perform) {
			const newLd = dabL.findRelocationPositionsHull(
				this.brushingStatus[this.currentBrushIdx], this.lensHull, this.relocationStartLd,
				this.closenessArr, this.painterRadius, this.canvasSize
			);
			this.nextLd = newLd.map(pos => [pos[0], pos[1]]);
			this.isRelocating = true;
		}
		else {
			this.nextLd = this.relocationStartLd.map(pos => [pos[0], pos[1]]);
			this.isRelocating = false;
		}
	...
	performRelocationDuringBrushing(relocationProgress) {
		this.isRelocating = true;
		const startLd = this.relocationStartLd ?? this.currLd;
		const progress = Math.max(0, Math.min(1, relocationProgress));
		const easedProgress = progress < 0.5
			? 4 * progress * progress * progress
			: 1 - Math.pow(-2 * progress + 2, 3) / 2;
		const intermediateLd = startLd.map((pos, i) => {
			const target = this.nextLd[i] ?? pos;
			const posX = pos[0] + (target[0] - pos[0]) * easedProgress;
			const posY = pos[1] + (target[1] - pos[1]) * easedProgress;
			return [posX, posY];
		});
	...
	completeRelocationDuringBrushing() {
		if (this.nextLd !== undefined) {
			this.currLd = this.nextLd.map(pos => [pos[0], pos[1]]);
		}
		this.relocationStartLd = undefined;
		this.isRelocating = false;
	}
			...
			(progress, relocationProgress) => {
				...
				if (progress > 1) {
					if (relocationProgress < 1) {
						...
						this.completeRelocationDuringBrushing();
					}
				}
				pr.scatterplotRenderer(
				...
				if (progress < 1) {
					if (!isResume || this.lensHull == undefined) this.lensRendering("circle", this.painterRadius, startBrushingXPos, startBrushingYPos, 1);
